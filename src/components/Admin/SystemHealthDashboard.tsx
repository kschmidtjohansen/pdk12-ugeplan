...
export const SystemHealthDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Supabase Issues Monitor */}
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Activity className="h-5 w-5" />
            Supabase Services Health Monitor
          </CardTitle>
          <CardDescription>
            Comprehensive monitoring and auto-resolution of Supabase service issues
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                {systemHealthScore >= 90 && <CheckCircle2 className="h-4 w-4 text-green-600" />}
                {systemHealthScore < 70 && <AlertTriangle className="h-4 w-4 text-red-600" />}
              </div>
            </div>
            
            <div className="flex flex-col space-y-2">
              <span className="text-sm font-medium">Total Issues Detected</span>
          ...
          <div className="flex gap-2 mb-4">
            ...
            >
              ...
              {isAuditing ? 'Running Audit...' : 'Run Health Audit'}
            </Button>
          ...
          {auditReport && (
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                {Object.entries(auditReport.services).map(([serviceName, serviceData]) => (
                  <Card key={serviceName} className="p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium capitalize">{serviceName}</span>
                      <Badge variant={
                        serviceData.status === 'healthy' ? 'default' : 
                        serviceData.status === 'warning' ? 'secondary' : 'destructive'
                      }>
                        {serviceData.status}
                      </Badge>
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {serviceData.issues.length} issues
                    </div>
                  </Card>
                ))}
              </div>
