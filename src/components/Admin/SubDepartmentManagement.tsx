...
const SubDepartmentManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Fetch departments the user has access to
      let deptQuery = supabase.from('departments').select('id, name').order('name');

      if (!isSuperAdmin) {
        // Non-super-admins: only show departments they have access to
        ...
        if (deptIds.length > 0) {
          ...
          // Also check home_department_id on profile
  ...
  const handleDelete = async () => {
    ...
    // Remove user_access references first
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('admin.subDepartments.deleteWarning')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
