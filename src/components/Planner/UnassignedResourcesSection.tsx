...
const UnassignedResourcesSection: React.FC<UnassignedResourcesSectionProps> = ({
  ...
  const formatDate = (date: Date) => {
    ...
    if (currentLanguage === 'da') {
      ...
      const dateFormatted = format(date, 'd. MMM yyyy', {
        locale
      }); // Day with period, month abbreviation
  ...
  const getUnassignedEmployees = (date: Date) => {
    ...
    return employees.filter(employee => employee.role === 'servicemedarbejder' && !assignedEmployeeNames.has(employee.name) && !employee.onLeave && !isEmployeeOnVacation(employee.id, date));
  };
  ...
  return <div className="border border-blue-200 rounded-lg p-4 mb-6 bg-polygon-light">
      ...
      {isExpanded && <div className="mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {unassignedEmployees.length === 0 ? <p className="text-sm text-gray-500">{t('planner.allEmployeesAssigned')}</p> : <div className="space-y-1">
                  {displayedEmployees.map(employee => <div key={employee.id} className="text-sm bg-white p-2 rounded border">
                      {employee.name}
                    </div>)}
                  {unassignedEmployees.length > 3 && <Button variant="ghost" size="sm" onClick={() => setShowAllEmployees(!showAllEmployees)} className="text-sm text-blue-600 hover:text-blue-800 p-1 h-auto">
                      {showAllEmployees ? t('planner.showLess') : `+${unassignedEmployees.length - 3} ${t('planner.showMore')}`}
                    </Button>}
                </div>}
            </div>
            ...
            <div>
              ...
              {unassignedCars.length === 0 ? <p className="text-sm text-gray-500">{t('planner.allCarsAssigned')}</p> : <div className="space-y-1">
                  {displayedCars.map(car => <div key={car.id} className="text-sm bg-white p-2 rounded border">
                      {car.car_number} - {car.name}
                    </div>)}
                  {unassignedCars.length > 3 && <Button variant="ghost" size="sm" onClick={() => setShowAllCars(!showAllCars)} className="text-sm text-blue-600 hover:text-blue-800 p-1 h-auto">
