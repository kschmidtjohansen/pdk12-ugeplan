...
import { Calendar, Clock, MapPin, Users, Car as CarIcon, UserCheck, Navigation, FileText, Edit3 } from 'lucide-react';
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {canEdit && onEdit && <Button variant="outline" size="sm" onClick={() => onEdit(assignment)} className="flex items-center gap-2">
        ...
        <div className="space-y-6">
          ...
          {assignment.case_number && <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
              <div>
              ...
              <OneDriveFolderButton caseNumber={assignment.case_number} size="md" showText={true} className="bg-white hover:bg-blue-100" />
            </div>}
          ...
          {/* Location */}
          {assignment.location && <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => handleNavigate(assignment.location)} className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            {/* Responsible User */}
            {assignment.responsibleUser?.name && <div className="flex items-center gap-3 p-3 bg-indigo-50 rounded-lg">
            ...
            {/* Assigned Employees */}
            {(assignment.assignedEmployees?.length || assignment.employees?.length) && <div className="space-y-2">
                <p className="text-sm text-muted-foreground">{t('planner.employeesLabel')}</p>
                <div className="flex flex-wrap gap-2">
                  {assignment.assignedEmployees?.length ? assignment.assignedEmployees.map(emp => <Badge key={emp.id} variant="secondary">
                        {emp.name}
                      </Badge>) : assignment.employees?.map((name, index) => <Badge key={index} variant="secondary">
                        {name}
                      </Badge>)}
                </div>
              </div>}
          ...
          {/* Cars */}
          {carNames.length > 0 && <div className="space-y-2">
              <div className="flex items-center gap-2">
                <CarIcon className="h-5 w-5 text-muted-foreground" />
                <p className="text-sm text-muted-foreground">{t('planner.carLabel')}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {carNames.map((carName, index) => <Badge key={index} variant="outline">
                    {carName}
                  </Badge>)}
              </div>
            </div>}
          ...
          {/* Description */}
          {assignment.description && <div className="space-y-2">
