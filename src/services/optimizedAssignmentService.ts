...
export class OptimizedAssignmentService {
  static async fetchAssignmentsWithFilter(filter: string, userId?: string, userRole?: string): Promise<OptimizedAssignmentData[]> {
    ...
    try {
      ...
      // DEFINITIVE FIX: Clear and correct filter logic
      if (filter === 'user' && userId) {
        // DASHBOARD: Get assignments where user is assigned OR responsible
        console.log('[OptimizedAssignmentService] DEFINITIVE FIX - User assignments for dashboard');
        ...
        // PLANNER: Show ALL published assignments - NO user filtering for servicemedarbejder
        console.log('[OptimizedAssignmentService] DEFINITIVE FIX - ALL published assignments (no user filtering)');
        assignmentsQuery = assignmentsQuery.eq('published', true);
        
      } else if (filter === 'all') {
        // PLANNER: Show ALL assignments for admin/skadeleder
        console.log('[OptimizedAssignmentService] DEFINITIVE FIX - ALL assignments for admin/skadeleder');
      ...
      // DEFINITIVE FIX: Get ALL employee relationships for ALL assignments
      ...
      // DEFINITIVE FIX: Get ALL employee profiles for complete names
      ...
      // DEFINITIVE FIX: Process assignments with ALL employee information preserved
      const result = assignments.map(assignment => {
        // Get ALL employees for this assignment (not just current user)
        ...
        // Debug logging for Asbestkursus assignments
        if (assignment.title.toLowerCase().includes('asbestkursus')) {
          console.log(`[OptimizedAssignmentService] DEFINITIVE FIX - 🎯 ASBESTKURSUS:`, {
            ...
            allRelations: assignmentEmployeeRelations.length
          });
        }
      ...
      console.log('[OptimizedAssignmentService] DEFINITIVE FIX - Final result:', {
        filter,
