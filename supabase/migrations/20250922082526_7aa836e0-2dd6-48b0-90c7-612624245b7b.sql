-- Update the list_accessible_assignments_with_team function to show all published assignments for servicemedarbejder users
-- This allows the planner page to display all assignments while maintaining client-side filtering for "Mine Opgaver"
...
BEGIN
  ...
  ELSE
    -- Servicemedarbejder: see ALL published assignments (not just assigned ones) for planner functionality
    -- Client-side filtering will handle showing only relevant assignments in specific widgets
    RETURN QUERY
    SELECT 
      a.id,
      a.title,
      ...
      a.created_at,
      a.updated_at,
      -- Get team data for all assignments
      COALESCE(
        (SELECT jsonb_agg(
          jsonb_build_object(
            'id', p.id,
    ...
    FROM public.assignments a
    LEFT JOIN public.profiles rp ON a.responsible_user_id = rp.id
    WHERE a.published = true  -- Show all published assignments for servicemedarbejder
    ORDER BY a.assignment_date DESC, a.from_time DESC;
