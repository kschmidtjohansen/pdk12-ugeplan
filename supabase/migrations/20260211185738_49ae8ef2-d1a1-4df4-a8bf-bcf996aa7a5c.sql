...
-- 1. Add department_id to cars table
ALTER TABLE public.cars ADD COLUMN IF NOT EXISTS department_id uuid REFERENCES public.departments(id);

-- 2. Replace the RPC function with p_department_id support and super_admin role
CREATE OR REPLACE FUNCTION public.list_accessible_assignments_with_team(p_department_id uuid DEFAULT NULL)
RETURNS TABLE (
  id uuid,
  title text,
  description text,
...
BEGIN
  ...
  PERFORM public.log_security_event_safe(
    ...
    format('User accessing assignments with role: %s, department: %s', current_user_role, p_department_id),
    jsonb_build_object(
