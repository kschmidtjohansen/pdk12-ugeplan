-- Phase 3: Department-Aware RLS Policies (Security) - FIXED
...
USING (
  ...
  profiles.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
);
...
WITH CHECK (
  ...
  EXISTS (
    ...
    AND ud.department_id = profiles.department_id
  )
);
...
USING (
  ...
  assignments.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
  AND (
    ...
    OR assignments.responsible_user_id = auth.uid()
...
FOR SELECT TO authenticated
USING (
  cars.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
);
...
USING (
  ...
  AND notifications.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
);

CREATE POLICY "notifications_insert_admin_or_system" ON public.notifications
FOR INSERT TO authenticated
WITH CHECK (
  (notifications.user_id = auth.uid() AND notifications.department_id = ANY(public.get_user_accessible_departments(auth.uid())))
  OR EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = auth.uid() 
    AND ur.role IN ('administrator', 'skadeleder')
...
FOR SELECT TO authenticated
USING (
  vacations.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
  AND (
    vacations.user_id = auth.uid()
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid() 
      AND ur.role IN ('administrator', 'skadeleder')
...
FOR INSERT TO authenticated
WITH CHECK (
  (vacations.user_id = auth.uid() AND vacations.department_id = ANY(public.get_user_accessible_departments(auth.uid())))
  OR EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = auth.uid() 
    AND ur.role IN ('administrator', 'skadeleder')
...
USING (
  vacations.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
  AND (
    (vacations.user_id = auth.uid() AND vacations.status = 'pending')
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid() 
      AND ur.role IN ('administrator', 'skadeleder')
...
USING (
  ...
  OR user_roles.department_id = ANY(public.get_user_accessible_departments(auth.uid()))
...
FOR SELECT TO authenticated
USING (
  assignments_employees.user_id = auth.uid()
  OR EXISTS (
    SELECT 1 FROM public.assignments a
    WHERE a.id = assignments_employees.assignment_id 
