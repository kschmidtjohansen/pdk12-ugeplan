...
-- Allow authenticated users to view all roles (simplified approach to avoid recursion)
CREATE POLICY "user_roles_all_select" ON public.user_roles
  FOR SELECT USING (auth.role() = 'authenticated');

-- Only allow service role to modify roles (for admin functions)
CREATE POLICY "user_roles_service_all" ON public.user_roles
  FOR ALL USING (auth.role() = 'service_role');

-- Update the helper functions to be more robust and avoid recursion
